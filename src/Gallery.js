...
const getLayerSize = (layerNo) =>
  (2 * layerNo + 1) ** 2 - (2 * layerNo - 1) ** 2;
...
const getItemNoInLayer = (itemNo, layerNo) =>
  itemNo - (2 * layerNo - 1) ** 2 + 1;
...
const getStaticAbsolutePosition = (
  ...
  const top =
    50 -
    (itemHeight / 2 + topIndex * verticalGap + (topIndex - 0.5) * itemHeight);
  const left =
    50 +
    itemWidth / 2 +
    leftIndex * horizontalGap +
    (leftIndex - 0.5) * itemWidth;
  return [top, left];
};

const getDynamicAbsolutePosition = (
  top,
  left,
  mouseTop,
  mouseLeft,
